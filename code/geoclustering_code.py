...
def initialization():
    ...
    df = pd.DataFrame(index=['map_parts_total', 'output_raster_columns', 'output_raster_rows',
                             # from cut_raster_file_to_smaller_boxes
...
def choose_reference_values(param, paths):
    ...
    for input_file in paths["inputs"]:
        ...
        logger.info(
            'Calculating relative size, relative std, product of relative size and relative std and four extreme '
        ...
        non_empty_rasters['ul_corner'] = np.exp(
            (-non_empty_rasters['rel_size'] + non_empty_rasters['rel_std']).astype(float))
        non_empty_rasters['ur_corner'] = np.exp(
            (non_empty_rasters['rel_size'] + non_empty_rasters['rel_std']).astype(float))
        non_empty_rasters['ll_corner'] = np.exp(
            (-non_empty_rasters['rel_size'] - non_empty_rasters['rel_std']).astype(float))
        non_empty_rasters['lr_corner'] = np.exp(
            (non_empty_rasters['rel_size'] - non_empty_rasters['rel_std']).astype(float))
        counter_files = chr(ord(counter_files) + 1)
...
def identify_number_of_optimum_clusters(param, paths):
    ...
    if "max_no_of_cl" in param:
        logger.info('The user has set the maximum number of clusters in the reference raster: %d.',
                    param["max_no_of_cl"])
        ...
        for input_file in paths["inputs"]:
            ...
            if len(data) == 0:
                ...
                data = data.join(
                    pd.DataFrame({'X': X, 'Y': Y, 'Value_' + counter_files: array_raster}).set_index(['X', 'Y']),
                    how='inner')
                counter_files = chr(ord(counter_files) + 1)
        ...
        for i in k_set:
            ...
            CL = kmeans.fit(coef)
            k_means_stats.loc[
                i, 'Inertia'] = kmeans.inertia_  # inertia is the sum of the square of the euclidean distances
            ...
            if i == k_set[0]:
                ...
                k_means_stats.loc[i, 'Slope'] = k_means_stats.loc[i, 'Distance'] - k_means_stats.loc[
                    i - k_step, 'Distance']
    ...
    for i in non_empty_rasters:
        ...
        for input_file in paths["inputs"]:
            ...
            if len(data) == 0:
                table = pd.DataFrame({'Value_' + counter_files: array_raster})
                data = pd.DataFrame(
                    {'X': X, 'Y': Y, 'Value_' + counter_files: array_raster[~np.isnan(array_raster)]}).set_index(
                    ['X', 'Y'])
        ...
        for input_file in paths["inputs"]:
            if param["agg"][counter_files] == 'density':
                ...
                for cl in table.loc[pd.notnull(table['Value_' + counter_files]), 'CL'].unique():
                    table.loc[table['CL'] == cl, 'Value_' + counter_files] = table.loc[
                        table['CL'] == cl, 'Value_' + counter_files].mean()
            else:
                for cl in table.loc[pd.notnull(table['Value_' + counter_files]), 'CL'].unique():
                    table.loc[table['CL'] == cl, 'Value_' + counter_files] = \
                    table.loc[table['CL'] == cl, 'Value_' + counter_files].param["agg"][counter_files]
...
def max_p_algorithm(param, paths):
    ...
    for i in non_empty_rasters:
        ...
        thr = (coef['a'] * (
            exp(-coef['b'] * (group_by_part.loc[i, 'rel_size'] + (coef['c'] * group_by_part.loc[i, 'rel_std']))))) * \
              data['Value'].sum() * 0.25
        ...
        r = ps.region.maxp.Maxp(w, data['Value'].values.reshape(-1, 1), floor=thr, floor_variable=data['Value'],
                                initial=100)
        ...
        for input_file in paths["inputs"]:
            if param["agg"][counter_files] == 'density':
                ...
                for cl in data.loc[pd.notnull(data['Value_' + counter_files]), 'CL'].unique():
                    data.loc[data['CL'] == cl, 'Value_' + counter_files] = data.loc[data[
                                                                                        'CL'] == cl, 'Value_' + counter_files].sum() / \
                                                                           data.loc[data['CL'] == cl, 'area'].sum()
            else:
                for cl in data.loc[pd.notnull(data['Value_' + counter_files]), 'CL'].unique():
                    data.loc[data['CL'] == cl, 'Value_' + counter_files] = \
                    data.loc[data['CL'] == cl, 'Value_' + counter_files].param["agg"][counter_files]
