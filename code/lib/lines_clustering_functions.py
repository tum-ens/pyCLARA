def cluster_trans_file(trans_file_path, voronoi_file_path, no_of_clusters=28):
    ...
    print("Assigning origin and destination polygons to transmission lines to determine if point is in ocean or on " "land.")
    ...
    # Creating geoDataFrame of points that make up the transmission lines.
    logger.info("Creating geoDataFrame of points that make up the transmission lines after filtering of lines that " "terminate in the ocean.")
    df_points_1 = gdf_trans["ID"].copy()
    df_points_2 = gdf_trans["ID"].copy()
    ...
    logger.info("Due to a bug in geopandas, setting the type of geoDataFrame changes it to DataFrame. Setting it back" " to a geoDataFrame.")
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        level=logging.DEBUG, format="%(asctime)s [%(levelname)s] - %(message)s", filename="log_connecting_islands.txt"
