...
def calculate_stats_for_non_empty_rasters(paths, param):
    ...
    for counter_files in range(len(paths["inputs"])):
        ...
        print('Calculating relative size, relative std, product of relative size and relative std and four extreme corners of data cloud for raster ' + raster_name)
...
def identify_opt_number_of_clusters(paths, param, part, size_of_raster, std_of_raster):
    ...
    if param["kmeans"]["method"] == "reference_part":
        ...
        optimum_no_of_clusters_for_raster = int(np.ceil(maximum_no_of_clusters * (coef_size * (size_of_raster / size_max)
                                                                               + coef_std * (std_of_raster / std_max))))
...
def identify_max_number_of_clusters_in_ref_part(paths, param):
    ...
    for counter_files in range(len(paths["inputs"])):
        ...
        if coef[raster_name].min() == coef[raster_name].max():
            ...
            coef[raster_name] = 0.1 / np.sqrt(n_cols) * param["weights"][counter_files] * (coef[raster_name] - coef[raster_name].min()) / (coef[raster_name].max() - coef[raster_name].min())
    ...
    for i in non_empty_rasters:
        ...
        for counter_files in range(len(paths["inputs"])):
            ...
            if len(data)==0:
                ...
                data = data.join(pd.DataFrame({'X': X, 'Y': Y, raster_name: array_raster[~np.isnan(array_raster)]}).set_index(['X', 'Y']), how='inner')
